...
def multi_classes_dice_loss(preds, targets, num_classes):
    ...
    for cls in range(num_classes):
        ...
        dice_loss += 1 - ((2 * intersection + 1e-6) / (union + 1e-6))  # Dice Loss计算公式
...
class TverskyLoss(nn.Module):
    def __init__(self, apply_nonlin=None, batch_dice=False, do_bg=True, smooth=1.,alpha=0.5,beta=0.5,
                 square=False):
        """
...
class FocalTversky_loss(nn.Module):
    ...
    def __init__(self, gamma=0.75,alpha = 0.5, beta = 0.5):
